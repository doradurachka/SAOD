...
mas_bin = matrix
start = time.time()
findBinarySearch(mas_bin, element)
end = time.time() - start
...
def fibonacciSearch(matrix, val):
    ...
    while fibthird > 1:
        ...
        if matrix[i] < val:
            ...
            return i
    if matrix[index + 1] == val and index < len(matrix) - 1:
        return index + 1
    return -1
...
mas_fib = matrix
start = time.time()
a = fibonacciSearch(matrix, element)
end = time.time() - start
if a == -1:
    print("Искомого числа в массиве нет")
else:
...
while True:
    try:
        k = int(input("Введите элемент который хотите найти "))
    except:
        print("Ошибка, введено не целое число")
        continue
    break
...
rand_re_hash(sl, matrix)
stroka = str(sl.items())
print(stroka)
m = interpolationSearch(matrix, k)
print(matrix)
print(m)